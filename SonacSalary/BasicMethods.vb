...
Public Class BasicMethods
    ...
    Sub AccNoLostFocus(ByVal txtId As TextBox, ByVal txtName As TextBox, Optional ByVal SubType As Integer = 1, Optional ByVal LinkFile As Integer = 0, Optional IgnoreVisibilty As Boolean = False)
        If txtId.Text.Trim = "" Or txtId.Text.Trim = "0" Or (txtId.Visibility = Visibility.Hidden And Not IgnoreVisibilty) Then
            txtId.Clear()
            txtName.Clear()
            Return
        End If
    ...
    Public Function AccNoShowHelp(ByVal txtId As TextBox, ByVal txtName As TextBox, ByVal e As System.Windows.Input.KeyEventArgs, Optional ByVal SubType As Integer = 1, Optional ByVal LinkFile As Integer = 0) As Boolean
        Return ShowHelp("الحسابات", txtId, txtName, e, "select Id,Name from Chart where SubType=" & SubType & " and (LinkFile=" & LinkFile & " or " & LinkFile & "=0)")
    End Function
    ...
    Public Function ShowHelp(ByVal Header As String, ByVal txtId As TextBox, ByVal txtName As TextBox, ByVal e As System.Windows.Input.KeyEventArgs, ByVal Statement As String, Optional ByVal TableName As String = "", Optional MyFields() As String = Nothing, Optional MyValues() As String = Nothing) As Boolean
    ...
    Public Function ShowHelpGrid(ByVal Header As String, ByVal Column1 As Forms.DataGridViewCell, ByVal Column2 As Forms.DataGridViewCell, ByVal e As Forms.KeyEventArgs, ByVal Statement As String, Optional ByVal TableName As String = "") As Boolean
    ...
    Public Function Protect(ByVal s As String) As String
        Return s.Replace(" ", "").Replace("0", "PE").Replace("1", "EK").Replace("2", "UR").Replace("3", "TG").Replace("4", "YR").Replace("5", "DU").Replace("6", "WO").Replace("7", "QL").Replace("8", "AJ").Replace("9", "ZS").ToUpper()
    End Function

    Public Function ProtectionSerial() As String
        Return Protect(Physics_processorId()).Trim() & Physics_BaseBoard()
    End Function
    ...
    Public Sub SearchTable(ByVal ID() As String, ByVal IDValue() As String, Cbo As ComboBox, Optional SpName As String = "")
        Try
            Dim str As String = ""
            ...
            For i As Integer = 1 To ID.Length - 2
...
EndFor:
        ...
        Return ExecuteNonQuery(Statement & Statement2 & Statement3)
    End Function
...
A:
    ...
    Public Function AddItemToTable(ByVal tbl As String, ByVal Fld As String(), ByVal str As String(), Optional ByVal p() As String = Nothing, Optional ByVal v() As String = Nothing) As Boolean
        ...
        For i As Integer = 0 To Fld.Length - 1
            ...
            str5 &= ",'" & str(i) & "'"
        Next
        Return ExecuteNonQuery("insert " & tbl & "(Id,UserName,MyGetDate" & str2 & str4 & ") values(isnull((select MAX(Id)+1 from " & tbl & " where 1=1 " & str1 & "),1)," & Md.UserName & ",GETDATE()" & str3 & str5 & ")")
    End Function
    ...
    Public Function Encrypt(ByVal plainText As String) As String
        ...
        passPhrase = "ddd@se"        ' can be any string
        saltValue = "N!1212@t"        ' can be any string
        hashAlgorithm = "MD5"             ' can be "MD5"
        passwordIterations = 53                  ' can be any number
    ...
    Sub ResetComboboxContent(cbo As ComboBox)
        ...
        For i As Integer = 0 To cbo.Items.Count - 1
            Try
                ...
                If CType(cbo.Items(i), ComboBoxItem).Content <> "" Then
